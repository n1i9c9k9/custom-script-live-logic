...
class LooperComponent(Component):
    ...
    def foot_switches(self, switch):
        if switch.index == self.BANKSWITCHUP:
            self.increment_currentBank(1)
        elif switch.index == self.BANKSWITCHDOWN:
            self.decrement_currentBank(1)
        else:
            if switch._double_click_context.click_count > 0:
                return
            track = self._tracks[self.updated_track_index(switch)]
    ...
    def foot_switches(self, switch):
        if switch.index == self.BANKSWITCHUP:
            self.increment_currentBank(4)
        elif switch.index == self.BANKSWITCHDOWN:
            self.decrement_currentBank(4)
        else:
            self._start_or_stop_playback(self._tracks[self.updated_track_index(switch)])
...
, chain(self._tracks[next_index:], self._tracks[:next_index])) or self._tracks[next_index])
    ...
    def updated_track_index(self, switch):
        return switch.index + self.currentBank
    
    def increment_currentBank(self, steps):
        self.currentBank = min(len(self.song.tracks) - 4, self.currentBank + steps)
    
    def decrement_currentBank(self, steps):
        self.currentBank = max(0, self.currentBank - steps)
